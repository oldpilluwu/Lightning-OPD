...
# End-to-end Qwen3-8B data curation for two separate Vast.ai servers,
# each with 8x RTX 3090 GPUs.
#
# Run the same script on both servers from the repository root:
#   NODE_RANK=0 bash scripts/curate_qwen3_8b_2x8x3090_vast.sh
#   NODE_RANK=1 bash scripts/curate_qwen3_8b_2x8x3090_vast.sh
#
# The two machines do not need to talk to each other. They deterministically
# process disjoint prompt shards using NODE_RANK / NUM_NODES and write one
# parquet shard per server.
...
require_gpu() {
    if ! command_exists nvidia-smi; then
        die "nvidia-smi was not found. Use a Vast.ai image with NVIDIA drivers/CUDA available."
    fi
    nvidia-smi >/dev/null || die "nvidia-smi failed. Check the NVIDIA driver/CUDA installation."
}

NUM_NODES="${NUM_NODES:-2}"
NODE_RANK="${NODE_RANK:?Set NODE_RANK=0 on the first Vast server and NODE_RANK=1 on the second.}"
if ! [[ "${NODE_RANK}" =~ ^[0-9]+$ ]]; then
    die "NODE_RANK must be an integer."
fi
if (( NODE_RANK < 0 || NODE_RANK >= NUM_NODES )); then
    die "NODE_RANK (${NODE_RANK}) must be in [0, $((NUM_NODES - 1))]."
fi
...
RAW_OUTPUT_DIR="${RAW_OUTPUT_DIR:-${ROOT_DIR}/data/sft_data/vast_2x8x3090_node${NODE_RANK}_raw}"
FINAL_PARQUET="${FINAL_PARQUET:-${ROOT_DIR}/data/sft_data/openthoughts3_${NUM_SAMPLES}_qwen3-8b_node${NODE_RANK}-of-${NUM_NODES}.parquet}"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${ROOT_DIR}/data/.pipeline_state/vast_2x8x3090_node${NODE_RANK}}"
VENV_DIR="${VENV_DIR:-${ROOT_DIR}/.venv-curation}"
PYTHON_VERSION="${PYTHON_VERSION:-3.12}"

# RTX 3090 defaults. Qwen3-8B fits as one fp16 replica per GPU, which gives
# the best aggregate throughput across two 8-GPU boxes. If you see OOMs on a
# specific Vast image, rerun with TP_SIZE=2 BATCH_SIZE=32 MAX_NUM_SEQS=32.
...
echo "  workers_this_node=$((NUM_GPUS / TP_SIZE))"
echo "  world_size=$(((NUM_GPUS / TP_SIZE) * NUM_NODES))"
...
if [[ "${FORCE}" == "1" ]]; then
    log "FORCE=1: removing previous generated artifacts for node ${NODE_RANK}"
...
if [[ -f "${FINAL_PARQUET}" && "${FORCE}" != "1" ]]; then
    log "Node parquet already exists: ${FINAL_PARQUET}"
    log "Nothing to do. Set FORCE=1 to regenerate this node from scratch."
    exit 0
fi
...
log "Starting node ${NODE_RANK}/${NUM_NODES} vLLM generation with Qwen3-8B"
...
log "Merging this server's Arrow shards into ${FINAL_PARQUET}"
python data_curation/merge.py \
    --input-dir "${RAW_OUTPUT_DIR}" \
    --output "${FINAL_PARQUET}"

if [[ "${CLEAN_RAW}" == "1" ]]; then
    log "CLEAN_RAW=1: removing raw Arrow shards for this node"
    find "${RAW_OUTPUT_DIR}" -name "*.arrow" -delete
    find "${RAW_OUTPUT_DIR}" -type d -name "rank*" -prune -exec rm -rf {} +
fi

log "Done on node ${NODE_RANK}"
log "Node parquet shard: ${FINAL_PARQUET}"
log "After both servers finish, download both node parquet shards and merge them with scripts/merge_vast_qwen3_8b_shards.py"

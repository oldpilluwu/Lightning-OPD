...
def parse_args():
    ...
    # OPD readiness probe (see trainium/opd_probe.py): student NLL / fwd-KL /
    # drift on a frozen set of teacher responses to OPD prompts.
    p.add_argument("--opd-probe-parquet", default=None,
                   help="Probe parquet from build_opd_probe.py; enables probing.")
    p.add_argument("--probe-every", type=int, default=5,
                   help="Probe every N optimizer steps (default 5).")
    p.add_argument("--probe-size", type=int, default=64,
                   help="Number of probe sequences (fixed across the run).")
    ...
    def tokenize_fn(batch):
        ...
        for messages in batch["messages"]:
            ...
            if len(ids) > args.cutoff_len:
                continue  # merge.py already filtered by response length; drop stragglers
